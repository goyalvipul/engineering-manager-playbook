...
* [cite_start]**Current OKR Progress:** Are our onboarding experiments actually moving the activation funnel metrics? [cite: 24]
...
* [cite_start]**Experimentation Planning:** If deploying new growth features, ensure the A/B test instrumentation and cohort analysis parameters are fully defined with the Data team before coding begins[cite: 20].

## 🚢 Release & Deployment
* [cite_start]All significant features must be deployed behind feature flags to enable progressive rollouts (e.g., 10% -> 50% -> 100% of user base)[cite: 23].
* [cite_start]Deployments should be fully automated via CI/CD pipelines[cite: 12]. If a deployment requires manual SSH or database hacking, the release is blocked until the pipeline is fixed.
